...
restart_worker(#state{port = OldPort, path = Path} = State) ->
  ...
  WorkerPath = code:lib_dir(php, priv),
  Port = erlang:open_port({spawn, WorkerPath++"/worker.php "++Path}, [use_stdio,binary,exit_status,{packet,4}]),
  io:format("Start PHP worker at path ~s~n", [Path]),
